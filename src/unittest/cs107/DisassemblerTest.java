...
public class DisassemblerTest {
	...
	public void testHalfwordPrinting() {
		...
		assertEquals("Halfword formatting is not correct.", hw.toString(), "0000FEED A5A5");
		studentTotal += 1;
	}
	...
	public void testHexFileGetRecordMethods() {
		...
		assertEquals("Record item \"LL\" isn't extracted properly.", hf.getDataBytesOfRecord(hexRecord), 16);
		studentTotal += 1;
		assertEquals("Record item \"AAAA\" isn't extracted properly.", hf.getAddressOfRecord(hexRecord), 9314);
		studentTotal += 1;
		assertEquals("Record item \"TT\" isn't extracted properly.", hf.getRecordType(hexRecord), 0);
		studentTotal += 1;
	}
